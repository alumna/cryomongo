class Mongo::Client
  ...
  private def execute_command(
    ...
    **args,
    ...
    # only when leftover was >0 at wrap (20ms Instant floor for kernel
    # bytes after leftover Instant 0). leftover 0 at wrap raises on read
    # without a last-read. Do not last-read past leftover Instant.
    # Do not check! here.
  ...
  private def wrap_command_io(connection : Mongo::Connection, deadline : Mongo::Deadline?) : Bool
    if (d = deadline) && !d.infinite?
      ...
      # is now. leftover Instant is the wait. leftover 0 at wrap still
