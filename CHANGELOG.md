...
**3.13.1:** Audit of leftover skips and GitHub pending lists. Almost all pending UTF files are `runOnRequirements` (wrong topology, `auth: false`, old `maxServerVersion`) or official `skipReason`. The leftover `skip_op` holes were UTF `dropDatabase` and GridFS `drop`. `Database#drop` and `Collection#drop` send those commands. GridFS `drop` now drops the files collection then the chunks collection and shares one CSOT deadline. CMAP `waitQueueSize` / `waitQueueMultiple` stay skipped (deprecated; the spec allows that skip). GitHub 3.13.1: standalone **2:01**, replica set **4:29**, sharded **8:11**, load-balanced **4:13** (0 failures). Sharded UTF had an ~18s gap after the last file: RTT `close` did not interrupt an in-flight hello (`interrupt_and_wake`). UTF files whose topologies exclude the current topology are not registered as Crystal examples (not `pending`). Newer-server files stay `pending`.
