...
Local Phase 3.12 (`CRYSTAL_WORKERS=2`, **905** examples, 0 failures): standalone **2:55 / 167 pending**, replica set **7:24 / 45 pending**, load-balanced **6:28 / 102 pending**. Sharded was not re-run locally (load-balanced covers mongos). GitHub Docker 3.12 still needs a PR.
...
* **compression:** snappy and zstd OP_COMPRESSED. Handshake lists names the driver can use. zstd reuses one compress context and output buffer under a mutex. GitHub installs `libzstd-dev`. URI `compressors=zlib,snappy,zstd` (zlib first on CI).
