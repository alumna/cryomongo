...
Phase 3 of the roadmap is done. Remaining 8.0 work is Phase 3.1–3.14. **3.2:** Monitor hello command and network errors. **3.1:** CMAP pool at discovery (`poolCreatedEvent` / `poolReadyEvent`, background `minPoolSize`). GitHub `crystal spec -Dpreview_mt -Dexecution_context` with `CRYSTAL_WORKERS=2` and `compressors=zlib` after 3.2 is **783** examples: standalone **2:02 / 209 pending** (1 error, hello-timeout streaming wait), replica set **6:08 / 94 pending** (same error), sharded **8:05 / 101 pending** (green), load-balanced **5:25 / 136 pending** (green). mongod 8.0 awaitable hello waits ~1s, so the awaitable socket extra is at least 1s on non-mongos.
...
* **scripts:** Topology helpers at `scripts/` (native and Docker). `load-balanced` maps mongos `loadBalancerPort` 27050/27051. `stop_all` waits, then SIGKILL leftover mongos / shard / config processes, and removes `/tmp/mongodb-*.sock` so systemd mongod can bind 27017.
...
* **sdam:** A socket timeout after handshake does not mark the server Unknown and does not clear the pool (slow operation, not a dead server). URI `serverMonitoringMode` is `auto` / `poll` / `stream` (default `auto`).
...
* **utf:** Outcome documents are read with sort `{_id: 1}`. Unified SDAM files that only need existing ops now run. Standalone / sharded / load-balanced topology-lifecycle files run. Unified `minPoolSize-error.json`, `hello-command-error.json`, and `hello-network-error.json` run. Still skipped on purpose until the hole is closed (see `FIXES.md` / `ROADMAP.md` Phase 3.1–3.14): interruptInUse, logging, handshake backpressure labels, concurrent shutdown extra Unknown, heartbeat events, replica-set topology-lifecycle (one-member `rs0` locally and on GitHub).
