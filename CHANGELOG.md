...
Phase 3 of the roadmap is done. Remaining 8.0 work is Phase 3.1–3.14. **3.4:** Handshake backpressure labels (`SystemOverloadedError` + `RetryableError`) on TCP / hello network and timeout. **3.3:** Monitor heartbeat events (`ServerHeartbeatStartedEvent` / `Succeeded` / `Failed`) and `serverMonitoringMode` UTF. **3.2:** Monitor hello command and network errors. **3.1:** CMAP pool at discovery (`poolCreatedEvent` / `poolReadyEvent`, background `minPoolSize`). Local `crystal spec -Dpreview_mt -Dexecution_context` with `CRYSTAL_WORKERS=2` after 3.4 is **783** examples: standalone **2:04 / 204 pending**, replica set **6:05 / 92 pending**, sharded **8:06 / 100 pending**, load-balanced **5:41 / 136 pending**. Push a PR so GitHub Docker confirms the matrix. mongod 8.0 awaitable hello waits ~1s, so the awaitable socket extra is at least 1s on non-mongos.
...
* **utf:** Outcome documents are read with sort `{_id: 1}`. Unified SDAM files that only need existing ops now run. Standalone / sharded / load-balanced topology-lifecycle files run, including SDAM `expectEvents`. Unified `minPoolSize-error.json`, `hello-command-error.json`, `hello-network-error.json`, `serverMonitoringMode.json`, handshake backpressure files, and the non-auth test in `pool-clear-min-pool-size-error.json` run. Still skipped on purpose until the hole is closed (see `FIXES.md` / `ROADMAP.md` Phase 3.1–3.14): interruptInUse, logging, concurrent shutdown extra Unknown, replica-set topology-lifecycle (one-member `rs0` locally and on GitHub).
