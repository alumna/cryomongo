...
Phase 3 of the roadmap is done. Remaining 8.0 work is Phase 3.1–3.14. **3.6:** Ignore stale-generation application errors so two concurrent shutdowns emit one Unknown and one `poolClearedEvent`. Unified `find-shutdown-error.json` and `insert-shutdown-error.json` run on standalone, replica set, and sharded. **3.5:** Pool clear with `interruptInUseConnections` closes in-use sockets. Unified `interruptInUse-pool-clear.json` runs on replica set and sharded. **3.4:** Handshake backpressure labels (`SystemOverloadedError` + `RetryableError`) on TCP / hello network and timeout. **3.3:** Monitor heartbeat events (`ServerHeartbeatStartedEvent` / `Succeeded` / `Failed`) and `serverMonitoringMode` UTF. **3.2:** Monitor hello command and network errors. **3.1:** CMAP pool at discovery (`poolCreatedEvent` / `poolReadyEvent`, background `minPoolSize`). Local `crystal spec -Dpreview_mt -Dexecution_context` with `CRYSTAL_WORKERS=2` after 3.6 is **783** examples: standalone **2:12 / 202 pending**, replica set **6:14 / 89 pending**, sharded **8:10 / 97 pending**, load-balanced **5:15 / 136 pending**. Push a PR so GitHub Docker confirms the matrix. mongod 8.0 default awaitable hello wait is ~1s; test topologies set `minWaitForStreamingHelloMillis=0`. The awaitable socket extra is still at least 1s on non-mongos when the spec sum is under 1s.
