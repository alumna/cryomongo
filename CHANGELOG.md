...
Number of examples after 3.9: **796** (785 after 3.8 plus 11 client bulkWrite prose).
...
* **utf:** Outcome documents are read with sort `{_id: 1}`. `$$type: double` matches `$numberDouble`. Unified SDAM files that only need existing ops now run. Standalone / sharded / load-balanced topology-lifecycle files run, including SDAM `expectEvents`. Unified `minPoolSize-error.json`, `hello-command-error.json`, `hello-network-error.json`, `serverMonitoringMode.json`, handshake backpressure files, the non-auth test in `pool-clear-min-pool-size-error.json`, `interruptInUse-pool-clear.json`, `find-shutdown-error.json`, `insert-shutdown-error.json`, `rediscover-quickly-after-step-down.json`, `replicaset-emit-topology-changed-before-close.json`, CRUD `*-let.json`, count / mapReduce files, GridFS `deleteByName.json` / `renameByName.json`, and client `bulkWrite` files run. Still skipped on purpose until the hole is closed (see `FIXES.md` / `ROADMAP.md` Phase 3.11–3.14): logging. Atlas Search index ops stay `SKIP_TEST`. Handshake-error and unified SDAM auth files run when the URI has credentials (3.10).
