...
Number of examples after 3.11: **859** (802 after 3.10 plus 22 CLAM, 2 CMAP UTF, 33 cmap-format). After 3.9: **796** (785 after 3.8 plus 11 client bulkWrite prose).
...
GitHub Docker - Phase 3.10: **802** examples; standalone **2:25 / 166 pending**, sharded **8:40 / 53 pending**, load-balanced **6:00 / 98 pending**, all 0 failures. Replica set 3.10 CI stopped in `docker-topology.sh` after `rs.initiate` (createUser used 27017 while it was a secondary). Local Phase 3.11: standalone **2:54 / 167 pending**, sharded **9:17 / 56 pending**, load-balanced **6:21 / 102 pending** (859 examples, 0 failures). GitHub 3.11 still needs a PR. Local 3.8 times were standalone **2:13 / 183 pending**, replica set **6:23 / 67 pending**, sharded **8:17 / 77 pending**, load-balanced **5:22 / 116 pending**.
...
* **gridfs:** `delete_by_name` and `rename_by_name`. Unified `deleteByName.json` and `renameByName.json` run.
...
* **utf:** Outcome documents are read with sort `{_id: 1}`. `$$type: double` matches `$numberDouble`. Unified SDAM files that only need existing ops now run. Standalone / sharded / load-balanced topology-lifecycle files run, including SDAM `expectEvents`. Unified `minPoolSize-error.json`, `hello-command-error.json`, `hello-network-error.json`, `serverMonitoringMode.json`, handshake backpressure files, the non-auth test in `pool-clear-min-pool-size-error.json`, `interruptInUse-pool-clear.json`, `find-shutdown-error.json`, `insert-shutdown-error.json`, `rediscover-quickly-after-step-down.json`, `replicaset-emit-topology-changed-before-close.json`, CRUD `*-let.json`, count / mapReduce files, GridFS `deleteByName.json` / `renameByName.json`, and client `bulkWrite` files run. Unified SDAM `logging-*.json`, CLAM (23 files), and CMAP UTF run when `runOnRequirements` match. `waitForEvent` also counts `connectionReadyEvent`, checkout started / failed, and `poolClosedEvent`. Find UTF passes `projection`, `max`, `min`, `return_key`, `show_record_id`. Atlas Search index ops stay `SKIP_TEST`. Handshake-error and unified SDAM auth files run when the URI has credentials (3.10). Remaining 8.0 holes are Phase 3.12–3.14 (`FIXES.md` / `ROADMAP.md`).
