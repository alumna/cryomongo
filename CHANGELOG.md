...
UTF holds one cluster lock per JSON file (CMAP cmap-format files and prose `failCommand` tests use the same lock). A per-test lock still let another file run `killAllSessions` / `failCommand` between tests, so retryable writes retried and GitHub replica set failed `expectEvents` (5 errors after the v0.17.0 markdown merge, then again after the per-test lock). Sending every `failPoint` to all replica-set members was wrong (extra Unknown / checkout events, leftover `failGetMoreAfterCursorCheckout`). A hello `failPoint` for an appName client that does not exist yet is also set on the URI seed, because that seed is often a secondary. `waitForPrimaryChange` waits until that new primary answers hello as writable. UTF event lists are pinned per client and appended under a mutex so a closed client's late callback cannot land in the next test.

## 0.17.0 - 2026-08-25

Phase 3 of the roadmap is done (through **3.13.3**). Next 8.0 work is Phase **3.14** (performance review).

**3.13.1:** Audit of leftover skips and GitHub pending lists. Almost all pending UTF files are `runOnRequirements` (wrong topology, `auth: false`, old `maxServerVersion`) or official `skipReason`. The leftover `skip_op` holes were UTF `dropDatabase` and GridFS `drop`. `Database#drop` and `Collection#drop` send those commands. GridFS `drop` now drops the files collection then the chunks collection and shares one CSOT deadline. CMAP `waitQueueSize` / `waitQueueMultiple` stay skipped (deprecated; the spec allows that skip). Wrong-topology UTF files are not registered as Crystal examples. GitHub after teardown fix: standalone **2:02 / 14 pending / 754 examples**, replica set **4:06 / 17 / 879**, sharded **6:49 / 17 / 868**, load-balanced **4:19 / 17 / 822** (0 failures). Same band as GitHub 3.12 (1:59 / 4:26 / 6:56 / 4:24), with extra real UTF tests. Spec `after_suite` no longer drops leftover databases (that was the ~18s gap after the last UTF file). CMAP admin client closes when CMAP examples finish. `Monitor#close` interrupts the RTT fiber before it waits for scan.

**3.13.3:** After a handshake or heartbeat network error, a pinned mongos is Unknown and the pool is paused. The driver waits until that address is a mongos again before `commitTransaction` / `abortTransaction`. It does not pick another mongos while the session is still pinned. After unpin, commit can still move to another mongos (recovery token). Load-balanced: a transaction pin whose pool generation is old is dropped so the next command checks out a new socket and retries the handshake. Unified `retryable-commit-handshake.json` and `retryable-abort-handshake.json` run. GitHub Docker: standalone **2:03 / 14 pending / 754 examples**, replica set **4:33 / 15 / 879**, sharded **7:10 / 15 / 868**, load-balanced **4:23 / 15 / 822**. 0 failures. Same band as 3.13.1 (2:02 / 4:06 / 6:49 / 4:19) and 3.12 (1:59 / 4:26 / 6:56 / 4:24). Replica set / sharded / load-balanced pending dropped by 2 (the handshake files run).
...
* **utf:** Outcome documents are read with sort `{_id: 1}`. `$$type: double` matches `$numberDouble`. Unified SDAM files that only need existing ops now run. Standalone / sharded / load-balanced topology-lifecycle files run, including SDAM `expectEvents`. Unified `minPoolSize-error.json`, `hello-command-error.json`, `hello-network-error.json`, `serverMonitoringMode.json`, handshake backpressure files, the non-auth test in `pool-clear-min-pool-size-error.json`, `interruptInUse-pool-clear.json`, `find-shutdown-error.json`, `insert-shutdown-error.json`, `rediscover-quickly-after-step-down.json`, `replicaset-emit-topology-changed-before-close.json`, CRUD `*-let.json`, count / mapReduce files, GridFS `deleteByName.json` / `renameByName.json`, client `bulkWrite` files, and `retryable-commit-handshake.json` / `retryable-abort-handshake.json` run. Unified SDAM `logging-*.json`, CLAM (23 files), and CMAP UTF run when `runOnRequirements` match. `waitForEvent` also counts `connectionReadyEvent`, checkout started / failed, and `poolClosedEvent`. Find UTF passes `projection`, `max`, `min`, `return_key`, `show_record_id`. Atlas Search index ops stay `SKIP_TEST`. Handshake-error and unified SDAM auth files run when the URI has credentials (3.10). Next 8.0 work is Phase 3.14 (`FIXES.md` / `ROADMAP.md`).
