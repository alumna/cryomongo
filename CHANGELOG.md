...
Phase 3 of the roadmap is done. Remaining 8.0 work is Phase 3.1–3.14. **3.2:** Monitor hello command and network errors. **3.1:** CMAP pool at discovery (`poolCreatedEvent` / `poolReadyEvent`, background `minPoolSize`). Local `crystal spec -Dpreview_mt -Dexecution_context` with `CRYSTAL_WORKERS=2` and `compressors=zlib` after 3.2 is **783** examples, 0 failures, 0 errors: standalone **2:00 / 209 pending**, replica set **6:06 / 94 pending**, sharded **8:12 / 101 pending**, load-balanced **5:45 / 136 pending**. GitHub Docker should confirm `hello-timeout.json` "Driver extends timeout while streaming" on mongod (this LXC skips that test on standalone / replica set).
