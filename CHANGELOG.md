...
- Receive `Message` / `OpMsg` / `OpReply` are classes. Copy the frame, then BufferPool checkin, then `BSON.view` of `OwnedReceive`. Walk `error?` through `pin.bytes`; hold the owner for the whole walk; read `@bytes.size` after fetch. BSON document is a class (bson.cr **0.9.3**). Do not clone every ok:1 hello. Do not `BSON.new(BSON)`
